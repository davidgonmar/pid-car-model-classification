...
def create_heatmap(size=224, model="ResNet50"):
    ...
    if model == "ResNet50":
        ...
        wheels = np.exp(-0.01 * ((x - 70) ** 2 + (y - 160) ** 2)) + np.exp(
            -0.01 * ((x - 154) ** 2 + (y - 160) ** 2)
        )
...
def apply_heatmap(img, heatmap, alpha=0.6):
    ...
    for i in range(3):
        result[:, :, i] = (
            img[:, :, i] * (1 - alpha * heatmap_colored[:, :, 3])
...
def main():
    ...
    # Add overall title
    plt.suptitle(
        "Comparación de Visualizaciones Grad-CAM para Diferentes Arquitecturas",
        fontsize=14,
        fontweight="bold",
        y=0.95,
    )
